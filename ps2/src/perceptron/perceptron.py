...
def predict(state, kernel, x_i):
    ...
    beta = state[0]
    x = state[1]
    predict = sign( sum([beta[j] * kernel(x_i, x[j]) for j in range( len(beta))]) )
    return predict

    # return 1 if predict >= .5 else 0
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    beta_curr = 0 + learning_rate * (y_i - predict(state, kernel, x_i))

    state[0].append(beta_curr)
