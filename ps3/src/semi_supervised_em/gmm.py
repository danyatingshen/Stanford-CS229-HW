...
def main(is_semi_supervised, trial_num):
    ...
    for i in range(K):
        if (i != K - 1):
            row_temp = x[index[i * member: (i + 1) * member], :]
        else:
            row_temp = x[index[i * member: m], :]

        mean_temp = np.mean(row_temp, axis=0)
        mu.append(mean_temp)
        sigma.append((row_temp - mean_temp).T.dot(row_temp - mean_temp) / row_temp.shape[0])
...
def run_em(x, w, phi, mu, sigma):
    ...
    it = 0
    ll = old_ll = None
    while it < max_iter and (old_ll is None or np.abs(ll - old_ll) >= eps):
        # *** START CODE HERE
        # (1) E-step: Update your estimates in w
        # (2) M-step: Update the model parameters phi, mu, and sigma
        # (3) Compute the log-likelihood of the data to check for convergence.
        # By log-likelihood, we mean `ll = sum_x[log(sum_z[p(x|z) * p(z)])]`.
        # We define convergence by the first iteration where abs(ll - old_ll) < eps.
        # Hint: For debugging, recall part (a). We showed that ll should be monotonically increasing.
        # *** END CODE HERE ***

        for i in range(K):
            w[:, i] = np.exp(-(1 / 2) * ((x - mu[i]).dot(np.linalg.inv(sigma[i])) * (x - mu[i])).sum(axis=1)) / (
                    np.linalg.det(sigma[i]) ** (1 / 2)) * phi[i]
        ...
        it += 1
        old_ll = ll
        pr_xz = np.zeros(w.shape)
        for i in range(K):
            pr_xz[:, i] = np.exp(-(1 / 2) * ((x - mu[i]).dot(np.linalg.inv(sigma[i])) * (x - mu[i])).sum(axis=1)) / (
                    np.linalg.det(sigma[i]) ** (1 / 2)) * phi[i]
        try:
            ll = np.sum(np.log(pr_xz))
        except:
            print("divded zero")
        # *** END CODE HERE ***
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        for i in range(K):
            ...
            sigma[i] = ((w[:, i][:, None] * (x - mu[i])).T.dot(x - mu[i]) + alpha * (x_tilde_i - mu[i]).T.dot(
                x_tilde_i - mu[i])) / (sum(w[:, i]) + alpha * sum(z_tilde == i))
