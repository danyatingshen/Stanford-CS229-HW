...
def create_dictionary(messages):
    ...
    result = [word for word, count in count.items() if count >= 5]
    return { word: index for index, word in enumerate(result) }
    # *** END CODE HERE ***
...
def transform_text(messages, word_dictionary):
    ...
    counts = [collections.Counter(get_words(message)) for message in messages]
    matrix = np.zeros((m,n), dtype = int)

    for index in range(m):
        for word, count in counts[index].items():
            if word in word_dictionary:
                matrix[index][word_dictionary[word]] += count
    return matrix
...
def fit_naive_bayes_model(matrix, labels):
    ...
    phi_k_y_1 = (matrix[labels == 1].sum(axis = 0) + 1) / (n + matrix[labels == 1].sum())
    phi_k_y_0 = (matrix[labels == 0].sum(axis = 0) + 1) / (n + matrix[labels == 0].sum())
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    sum_of_log_p_x_y_1 = (np.log(phi_k_y_1) * matrix).sum(axis = 1) + np.log(phi_y)
    sum_of_log_p_x_y_0 = (np.log(phi_k_y_0) * matrix).sum(axis = 1) + np.log(1 - phi_y)
    result = (sum_of_log_p_x_y_1 > sum_of_log_p_x_y_0).astype(np.int)
    return result
    # *** END CODE HERE ***
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    # *** START CODE HERE ***
    phi_y, phi_k_y_1, phi_k_y_0 = model
    map = {value : key for key, value in dictionary.items()}
    result = list()
    for index in np.argsort(-(np.log(phi_k_y_1) - np.log(phi_k_y_0)))[:5]:
...
def compute_best_svm_radius(train_matrix, train_labels, val_matrix, val_labels, radius_to_consider):
    ...
    """
    # *** START CODE HERE ***
    result = radius_to_consider[0]
    accuracy = 0.0

    for r in radius_to_consider:
        accuracy = np.mean(svm.train_and_predict_svm(train_matrix, train_labels, val_matrix, r) == val_labels)
...
def compute_best_logreg_learning_rate(train_matrix, train_labels, val_matrix, val_labels, learning_rates_to_consider):
    ...
    """
    # *** START CODE HERE ***
    result = learning_rates_to_consider[0]
    accuracy = 0.0

    for rate in learning_rates_to_consider:
        accuracy = np.mean(logreg.train_and_predict_logreg(train_matrix, train_labels, val_matrix, rate) == val_labels)
        if accuracy > accuracy:
            result = rate
            accuracy = accuracy
