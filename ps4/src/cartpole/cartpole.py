...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    transition = mdp_data['transition_counts']
    next_states = transition.shape[0]
    reward = mdp_data['reward_counts']
    counts = transition.sum(axis=1)

    for i in range(next_states):
        for j in range(2):
            if counts[i, j] != 0:
                mdp_data['transition_probs'][i, :, j] = (transition[i, :, j]) / (counts[i, j])

    for i in range(next_states):
        sum = reward[i, 1]
        if sum != 0:
            mdp_data['reward'][i] = -reward[i, 0] / sum
    # *** END CODE HERE ***
    # This function does not return anything
    return
